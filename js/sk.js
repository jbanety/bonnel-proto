...
(function(sk, $) {
     ...
     *  - jquery.form.js, https://github.com/malsup/form/ ( pro odesÃ¡lÃ¡nÃ­ formulÃ¡Å™Å¯ )
     *
     * todo
     *    - otestovat a pÅ™ipravit ajaxify na formulÃ¡Å™ s file inputem
     */
    ...
    _fn.init = function() {
        ...
        if (History.enabled === false || ( this.options.pushStateOnly && History.emulated.pushState)) {
            this.log('historie v prohlÃ­Å¾eÄi nenÃ­ povolena')
            return this;
        }

        if (!this.options.pageTree || !this.options.pageTree.length) {
            this.log('nenÃ­ pÅ™edÃ¡n strom url webu')
            return this;
        }
    ...
    _fn.getPage = function(url) {
        ...
        if (typeof page === 'undefined') {
            ...
            // Pokud jsem naÅ¡el nejbliÅ¾Å¡Ã­ strÃ¡nku
            if (page) {
    ...
    _fn.initState = function() {
        ...
        if (!page) {
            this.log('aktuÃ¡lnÃ­ url nenÃ­ ve stromu strÃ¡nek')
        }
    ...
    _fn.pushState = function(state, title, href) {
        // udÃ¡lost a poslÃ¡nÃ­ objektu pro pÅ™Ã­padnou modifikaci
    ...
    _fn.changeState = function() {
        ...
        if ($target.is('form')) {
            ...
            this.ajax =
                $.ajax(this.state.data.ajax).done($.proxy(this.ajaxDone, this)).fail($.proxy(this.ajaxFail, this)).always($.proxy(this.ajaxEnd, this));
        }
    };
    ...
    _fn.getUrl = function($element) {
        ...
        // pokud je element form vytvoÅ™Ã­m si a pro parsovÃ¡nÃ­ vlastnostÃ­
        // if( $element.is('form') )
        // {
        // 	created = true;
...
(function(sk, $) {
    ...
    _fn.load = function(arr) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            (function(that, item) {
                $.when(that.loadItem(item)).done(function() {
                        loaded++;
                        tested++;
                    }).fail(function() {
                        error++;
    ...
    /**
     *    @public
     *    @param {String} KlÃ­Ä kterÃ½ chceme cachovat
     */
    ...
    /**
     *    @public
     *    @param {String} KlÃ­Ä k hodnotÄ› v cache
     *    @return {Boolean}
     */
...
(function(sk, $) {
    ...
    _fn.init = function() {
        ...
        this.$w.on('resize.MediaQueries', $.proxy(this.check, this)).trigger('resize');

        return this;
    };

    _fn.destroy = function() {
        this.$w.off('.MediaQueries');
            ...
            var fn = function() {
                ...
                if (query !== un) {
                    if (that.state === query.state) {
                        var e = $.Event('resizeMedia', {
                            state: this.state,
                            params: query.params
                        });
                        $(that).trigger(e);
                    } else {
                        var e = $.Event('changeMedia', {
                            state: query.state,
                            fromState: that.state,
                            params: query.params
                        });
...
(function(sk, $) {
    ...
    sk.widgets.Carousel = function(element, options) {
        ...
        this.options = $.extend({
            ...
            item: '>*', // poÄet elementÅ¯ k kterÃ© se bude posouvat
            scroll: 1, // aktuÃ¡lnÃ­ krok
            position: 0, // fullscreen reÅ¾im, bude scrollovat o procenta
            fullscreen: true, // skok na zaÄÃ¡tek
            repeat: false, // nekoneÄnÃ© scrollovÃ¡nÃ­
            infinite: false, // Äas za kterÃ½ se provede animace kroku. 0 vypne animaci a bude jen pÅ™epÃ­nat
            animation: 500, // efekt animace
            easing: Linear.easeNone, // osa
            axis: 'x', // x, y
            // ovlÃ¡dacÃ­ prvek pro pÅ™edchozÃ­
            pagerPrev: null, // ovlÃ¡dacÃ­ prvek pro dalÅ¡Ã­
            pagerNext: null, // vygenerovat menu pro kroky
            pagerPages: null, // maska krokÅ¯
            pagerMask: null,
            pagerMaskString: '{$active}/{$all}', // {$active} = aktivnÃ­ poloÅ¾ka, {$all} = celkovÃ½ poÄet
            // timeout po kterÃ© se automatiky pÅ™epne na dalÅ¡Ã­ krok. 0 znamenÃ¡ Å¾e se automaticky neposunuje
            timeout: 0, // callback
            onChange: null, // offset
            offset: 0, // prÃ¡zdnej element pro doplnÄ›nÃ­ kroku
    ...
    _fn.init = function() {
        ...
        this.$holder =
            this.$element[this.axProps.scrollLeft](0).wrap('<div class="sk-carousel sk-carousel-' + o.axis + '"></div>').parent();
        ...
        if (typeof sk.widgets.Pager !== 'undefined') {
            ...
            $(this.pager).on('change', $.proxy(function(e, obj) {
                    ...
                    this.handled = true;
                if (obj.prev) {
                    var pos = this.infinite ? (this.position - 1) : obj.current;
                }
                if (obj.next) {
                    var pos = this.infinite ? (this.position + 1) : obj.current;
                }
        ...
        if (this.max) {
            ...
            // callback
            typeof o.onStart == 'function' && o.onStart.call(this, {
    ...
    _fn.setPosition = function(pos, animate, init, animCall) {
        ...
        if (this.position != pos || init || animCall) {
            ...
            if (!animCall) {
                // pÅ™evrÃ¡tit pozice pokud je opakovanÃ­
                if (this.infinite) {
            ...
            if (!init && !animCall) {
                // callback
                typeof o.onChange == 'function' && o.onChange.call(this, {
                    position: pos,
                    handled: this.handled
                });
            }
    ...
    // vytvoÅ™enÃ­ nekoneÄnÃ©ho carouselu
...
(function(sk, $) {
    ...
    sk.widgets.Countdown = function(element, options) {
        ...
        this.options = $.extend({
            // defaultnÃ­ tÅ™Ã­da widgetu
            widgetClass: 'sk-countdown', // html widgetu
            widgetHtml: '<span class="sk-countdown-bar"></span>', // selekce elemetnÅ¯ kterÃ© se v Äase mÄ›nÃ­
            barSelector: '.sk-countdown-bar', // kolik Äasu se mÃ¡ odpoÄÃ­tÃ¡vat
            time: 7000
        }, options);
    ...
    _fn.init = function() {
        ...
        this.$element.addClass(o.widgetClass).append(o.widgetHtml);
    ...
    _fn.destroy = function() {
        ...
        this.$element.removeClass(o.widgetClass).empty();
    ...
    _fn.start = function() {
        ...
        this.anim =
            $.Animation({prc: this.prc}, {prc: 100}, {
                duration: time,
                easing: 'linear'
            });
...
(function(sk, $) {
    ...
    /*
     * VytvoÅ™Ã­ kruhovÃ½ odpoÄet Äasu. FunkÄnÃ­ je v prohlÃ­Å¾eÄÃ­ch podporujÃ­cÃ­ transformace
    ...
    sk.widgets.CountdownCircle = function(element, options) {
        var options = $.extend({
            // defaultnÃ­ tÅ™Ã­da widgetu
            widgetClass: 'sk-countdown-circle', // html widgetu
            widgetHtml: '\
				...
				<span class="sk-countdown-clip"> \
					<span class="sk-countdown-bar"></span> \
				</span>', // selekce elemetnÅ¯ kterÃ© se v Äase mÄ›nÃ­
            barSelector: '.sk-countdown-bar', // kolik Äasu se mÃ¡ odpoÄÃ­tÃ¡vat
            time: 7000,
        }, options);
...
(function(sk, $) {
    ...
    sk.widgets.Pager = function(options) {
        this.options = $.extend({
            // aktivnÃ­
            current: 0, // poÄet strÃ¡nek
            max: 0, // krok
            step: 1, // nekoneÄnÃ©
            infinite: false, // ovlÃ¡dacÃ­ prvek pro pÅ™edchozÃ­
            pagerPrev: null, // ovlÃ¡dacÃ­ prvek pro dalÅ¡Ã­
            pagerNext: null, // vygenerovat menu pro kroky
            pagerPages: null,
            pagerTemplate: '<a href="#"><span>{$index}</span></a>', // maska krokÅ¯
            pagerMask: null,
            pagerMaskString: '{$active}/{$all}', // {$active} = aktivnÃ­ poloÅ¾ka, {$all} = celkovÃ½ poÄet
            pagerMaskAdd: 1
